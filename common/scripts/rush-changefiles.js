...
const leadingEmojiRegex = /^[👷🔧💚📝✨🐛⚡♻\ufe0f⏪🎨✅]{1,2}/u;

function stripLeadingEmoji(commit) {
  return commit.replace(leadingEmojiRegex, "").trim();
}
...
function parseLastCommit(repoPath) {
  ...
  const { increment } = recommendedBump([lastCommit[0].rawBody].map(stripLeadingEmoji));
...
function parseRecentCommits(projectName, projectPath, lastCommitInfo, repoPath, defaultCommitMessage) {
  ...
  const commitMsgPass =
    ((commits.length == 2 && commits[0].rawBody != commits[1].rawBody) || commits.length == 1) &&
    commits[0].body != defaultCommitMessage;
  ...
  //no changes for this project were included in the last commit, or the last 2 commits identical
    ...
    if (!commitMsgPass) {
      console.log(
        utils.Colors.Gray +
          "The last two commit messages are the same, skipping change file generation" +
          utils.Colors.Reset
      );
    }
    if (!projectInCommit) {
      console.log(
        utils.Colors.Gray +
          `Project ${projectName} is not included in the current commit` +
          utils.Colors.Reset
      );
    }
    return false;
...
async function getChangedProjectNamesAsync() {
  ...
  try {
    ...
    if (currentBranch) {
      ...
      console.log(
        utils.Colors.Green + `Found changes in ${rushProjects.size} project(s)` + utils.Colors.Reset
      );
    }
    return rushProjects;
  } catch (error) {
...
function createChangeFile(rushConfig, bumpInfo) {
  let changeFilePath = rushLib.ChangeManager.createEmptyChangeFiles(
    rushConfig,
...
function generateChangeFiles(rushConfig, bumpInfo, projectInfo) {
  console.log(
    utils.Colors.Green +
      `Generating change file for "${bumpInfo.increment}": "${bumpInfo.subject}" for project ${bumpInfo.projectName}` +
      utils.Colors.Reset
  );
  ...
  projectInfo.consumingProjects.forEach((value, key) => {
    ...
    console.log(
      utils.Colors.Green +
        `Generating change file for "${patchInfo.increment}": "${patchInfo.subject}" for project ${patchInfo.projectName}` +
        utils.Colors.Reset
    );
  ...
  if (lastCommitInfo) {
    ...
    getChangedProjectNamesAsync().then((changedProjects) => {
      ...
      changedProjects.forEach((value, key) => {
        ...
        const result = parseRecentCommits(
          key,
          value.projectFolder,
          lastCommitInfo,
        ...
        if (result) {
          ...
          console.log(
            utils.Colors.Yellow + `Change file not required for project ${key}.` + utils.Colors.Reset
          );
        }
      });
      if (fileAdded) {
