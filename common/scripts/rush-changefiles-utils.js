  ...
  getCurrentBranch() {
    ...
    try {
      return child_process
        .execSync(`git rev-parse --symbolic-full-name --abbrev-ref "${currBranch}@{u}"`)
        .toString()
        .trim();
    } catch (error) {
      console.log(
        Colors.Red +
          "Error fetching git remote branch features/versions. Detected changed files may be incorrect." +
          Colors.Reset
      );
      console.log(
        Colors.Yellow +
          `Execute 'git push --set-upstream ${defaultRemote} ${currBranch}' or 'git checkout --track ${defaultRemote}/${currBranch}' to set upstream branch` +
          Colors.Reset
      );
      return null;
    }
